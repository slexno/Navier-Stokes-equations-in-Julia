...
function cavity_solver()
    ...
    CFL = 0.5
    F = 0.5
    Niter = 3000      # [iteration] Total number of iteration
    ...
    for it in 1:Niter # <--- Start of the temporal loop
        ...
        if it%N_out == 0
            interp_center!(ux_c,uy_c, ux_star,uy_star)
            u_mag_pred = (ux_c.^2 + uy_c.^2).^0.5
            p_pred = contourf(x_center, y_center, u_mag_pred', xlabel="x", ylabel="y", title="Prediction step velocity magnitude (it=$it)", color=:jet)
            frame(anim_pred, p_pred)
        end

        compute_pressure!(B, div_u, p, A, ux_star, uy_star, Nx, Ny, dx, dy, dt, rho)
        
        if it%N_out == 0
            p_press = contourf(x_center, y_center, p', xlabel="x", ylabel="y", title="Pressure field after pressure solve (it=$it)", color=:balance)
            frame(anim_press, p_press)
        end
        correction_step!(ux,uy,p,ux_star, uy_star, Nx, Ny, dx,dy,dt, rho)
        if it%N_out == 0
            interp_center!(ux_c,uy_c, ux,uy)
            u_mag_corr = (ux_c.^2 + uy_c.^2).^0.5
            p_corr = contourf(x_center, y_center, u_mag_corr', xlabel="x", ylabel="y", title="Correction step velocity magnitude (it=$it)", color=:jet)
            frame(anim_corr, p_corr)
        end
    end # <--- End of the temporal loop
    gif(anim_pred, "prediction_step.gif", fps=8)
    gif(anim_press, "pressure_step.gif", fps=8)
    gif(anim_corr, "correction_step.gif", fps=8)
...
function interp_center!(ux_c,uy_c, ux,uy)
    ...
    @inbounds for i in 1:Nx
        for j in 1:Ny
            ux_c[i,j] = (ux[i+1,j]+ux[i,j])/2
            uy_c[i,j] = (uy[i,j+1]+uy[i,j])/2
        end
    end
...
function convective_flux!(Flux_ux,Flux_uy, ux,uy, Nx, Ny, dx, dy)
    ...
    @inbounds for i in 2:Nx
        for j in 1:Ny
            ...
            Ue = 0.5*(ux[i, j] + ux[i+1, j])         # East normal velocity
            Uw = 0.5*(ux[i-1, j] + ux[i, j])         # West normal velocity
            Vn = 0.5*(uy[i-1, j+1] + uy[i, j+1])         # North normal velocity
            Vs = 0.5*(uy[i-1, j] + uy[i, j])     # South normal velocity
    ...
    @inbounds for i in 1:Nx
        for j in 2:Ny
            ...
            # 1) Normal velocity
            # ==================
            Ue = 0.5*(ux[i+1, j] + ux[i+1, j-1])     # East normal velocity
            Uw = 0.5*(ux[i, j-1] + ux[i, j])         # West normal velocity
            Vn = 0.5*(uy[i, j] + uy[i, j+1])         # North normal velocity
            Vs = 0.5*(uy[i, j-1] + uy[i, j])         # South normal velocity

            # ============
            # 2) Convective flux (Upwind Scheme)
            # ============

            uN = (Vn ≥ 0) ? uy[i,   j] : uy[i, j+1]  # North (No BC)
            uS = (Vs ≥ 0) ? uy[i, j-1] : uy[i,   j]  # South (No BC)

            if i != Nx
                uE = (Ue ≥ 0) ? uy[i,   j] : uy[i+1, j]  # East 
            else
                uE = (Ue ≥ 0) ? uy[i,   j] : uy_right  # East BC
            end

            if i != 1
                uW = (Uw ≥ 0) ? uy[i-1, j] : uy[i,   j]  # West
            else
                uW = (Uw ≥ 0) ? uy_left : uy[i,   j]  # West BC
            end
    ...
    @inbounds for i in 2:Nx
        for j in 1:Ny
            ...
            if j == 1
                ...
                grad_ux_S = (ux[i,j]   - ux_bottom) / (dy/2)
            elseif j == Ny
                grad_ux_N = (ux_top - ux[i,j]) / (dy/2)
                grad_ux_S = (ux[i,j]   - ux[i,j-1])/dy
            else
                grad_ux_N = (ux[i,j+1] - ux[i,j]) / dy
                grad_ux_S = (ux[i,j]   - ux[i,j-1])/dy
            end
    ...
    @inbounds for i in 1:Nx
        for j in 2:Ny
            ...
            if i == 1
                ...
                grad_uy_W = (uy[i,j]   - uy_left) / (dx/2)
            elseif i == Nx
                grad_uy_E = (uy_right - uy[i,j]) / (dx/2)
                grad_uy_W = (uy[i,j]   - uy[i-1,j]) / dx
            else
                grad_uy_E = (uy[i+1,j] - uy[i,j]) / dx
                grad_uy_W = (uy[i,j]   - uy[i-1,j]) / dx
            end
...
t_end = time()
println("Temps total d'exécution : ", t_end - t_start, " secondes")